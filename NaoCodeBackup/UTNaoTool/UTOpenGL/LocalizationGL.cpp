...
void LocalizationGL::drawParticles(const std::vector<Particle>& particles) {
  for(const auto& p : particles) {
    ...
    // basicGL.drawArrow(start, end, Colors::Indigo, Colors::Red, p.w, 5); // Based on weight
    basicGL.drawArrow(start, end, Colors::Indigo, Colors::Red, 1, 5); // Just draw particles 
  }
}
