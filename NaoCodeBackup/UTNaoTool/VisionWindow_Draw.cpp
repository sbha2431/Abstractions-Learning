...
void VisionWindow::drawRobot(ImageWidget* image) {
  ...
  auto& robot = world_object_block_->objects_[WO_TEAM1];
  ...
  int width = cmatrix.getCameraWidthByDistance(robot.visionDistance, 250);
  int height = cmatrix.getCameraHeightByDistance(robot.visionDistance, 450);
  int x1 = robot.imageCenterX - width / 2;
  
  // Draw top
  int ty1 = robot.imageCenterY - height;
  QPainterPath path;
  path.addRoundedRect(QRect(x1, ty1, width, height), 5, 5);
  painter.setPen(pen);
  painter.fillPath(path, QBrush(segCol[c_ROBOT_WHITE]));

}

void VisionWindow::drawLine(ImageWidget* image) {
  if(!config_.all) return;
  if(world_object_block_ == NULL) return;
  ...
  auto& line = world_object_block_->objects_[WO_OWN_PENALTY];
  if(not line.seen) return;
  if(line.fromTopCamera and _widgetAssignments[image] == Camera::BOTTOM) return;
  if(not line.fromTopCamera and _widgetAssignments[image] == Camera::TOP) return;

  QPen pen(segCol[c_WHITE]);

  int width = cmatrix.getCameraWidthByDistance(line.visionDistance, 700);
  int height = cmatrix.getCameraHeightByDistance(line.visionDistance, 20);
  int x1 = line.imageCenterX - width / 2;
  
  // Draw top
  int ty1 = line.imageCenterY - height;
  QPainterPath path;
  path.addRoundedRect(QRect(x1, ty1, width, height), 5, 5);
  painter.setPen(pen);
  painter.fillPath(path, QBrush(segCol[c_WHITE]));
