...
def setWalktoTarget(relX,relY,relTheta):
    walk_request.setWalkTarget(relX,relY,relTheta,True)


def setWalkPedantic(velX, velY, velTheta):
    scale = max(math.sqrt(velX * velX + velY * velY), 1.0)
    velX /= scale
    velY /= scale
    walk_request.setPedanticWalk(velX, velY, velTheta)
...
def setLShoulderRoll(target_pos, target_time=2.0, isChange=None):
    """Set head pan (left-right)."""
    if isChange is None:
        isChange = False
    joint_commands.setLShoulderRoll(target_pos, target_time * 1000.0, isChange)

def setRShoulderRoll(target_pos, target_time=2.0, isChange=None):
    """Set head pan (left-right)."""
    if isChange is None:
        isChange = False
    joint_commands.setRShoulderRoll(target_pos, target_time * 1000.0, isChange)

def setLShoulderPitch(target_pos, target_time=2.0, isChange=None):
    """Set head pan (left-right)."""
    if isChange is None:
        isChange = False
    joint_commands.setLShoulderPitch(target_pos, target_time * 1000.0, isChange)

def setRShoulderPitch(target_pos, target_time=2.0, isChange=None):
    """Set head pan (left-right)."""
    if isChange is None:
        isChange = False
    joint_commands.setRShoulderPitch(target_pos, target_time * 1000.0, isChange)

def setBShoulderPitch(target_pos, target_time=2.0, isChange=None):
    """Set head pan (left-right)."""
    if isChange is None:
        isChange = False
    joint_commands.setBShoulderPitch(target_pos, target_time * 1000.0, isChange)
