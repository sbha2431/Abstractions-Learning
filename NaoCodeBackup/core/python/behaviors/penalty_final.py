...
class CircleWalk(Node):
    ...
    def run(self):
        ...
        commands.setWalkVelocity(0,0.35,-0.15)
        commands.setHeadPan(0,0.1)
...
class Offense(Node):
    ...
    def run(self):
        ...
        if any(beacon_seen_anti) or self.antiFlag:
            self.antiFlag = True
        else:
            self.antiFlag = False
        ball = memory.world_objects.getObjPtr(core.WO_BALL)
        maxYaw = 1.0
        minYaw = -1.0
        if self.b_time <2.0 and self.walkFlag and not self.kickFlag:
            if err_dist<=0:
        ...
        elif self.b_time <2.0 and not self.kickFlag and self.circFlag:
            if self.antiFlag:
                if goal.seen:
                    print("goal angle ",goal.visionBearing)
                    if abs(goal.visionBearing)<=0.05:
                        commands.setWalkVelocity(0,0,0)
                        self.circFlag = False
                ...
                if goal.seen:
                    ...
                    self.postSignal("circle")
        elif self.b_time <2.0 and not self.kickFlag and not self.circFlag:
            print("Approaching ball")
            if goal.visionDistance<= 1400 or self.appFlag:
                self.appFlag = True
                if dist_error<= 6 and err_t2<=0.15:
                    print("Ready to kick. Ball at: ",ball.imageCenterX,", ",ball.imageCenterY)
                    commands.setWalkVelocity(0,0,0)
...
class ApproachToLine(Node):
  ...
  def run(self):
      ...
      if line.visionDistance < 120 and not (dist_error<= 6 and err_t2<=0.15 ):
        ...
        # print('x vel', w_dot)
        commands.setWalkVelocity(max(min(w_dot,max_speed),0),0,0)
...
class Set(LoopingStateMachine):
  def setup(self):
      ...
      for name in blocks:
          ...
          print("Block module: ",b)
          if name in savelist:
            self.add_transition(defense, S(name), b, T(2.1), defense)            
          else:
