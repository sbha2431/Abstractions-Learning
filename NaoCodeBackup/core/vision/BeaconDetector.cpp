...
void BeaconDetector::findBeacons(vector<Blob> &blobs) {
    ...
    for(auto beacon : beacons) {
        ...
        // std::cout<<"Beacon angle "<<cmatrix_.bearing(position) << endl;
        if (abs(cmatrix_.bearing(position))<2.0*M_PI/3.0){
            object.visionDistance = 0.95*bdistance; // Scale factor
            object.visionBearing = cmatrix_.bearing(position);
            object.seen = true;
            object.fromTopCamera = (camera_ == Camera::TOP);
        }
        else{
            // std::cout<<"Ommitted weird beacon"<<endl;
            // std::cout<<"Beacon angle "<<cmatrix_.bearing(position) << endl;
            object.seen = false;
            continue;
            // object.visionDistance = bdistance;
            // object.visionBearing = M_PI - cmatrix_.bearing(position);
            // object.seen = true;
            // object.fromTopCamera = (camera_ == Camera::TOP);
        }
