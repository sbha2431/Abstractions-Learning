...
#include <iostream>
#include <cmath>
#include <vision/structures/Blob.cpp>
...
void ImageProcessor::detectBall() {
    ...
    Position p = cmatrix_.getWorldPosition(ballc->centerX, ballc->centerY,0);
    ball->loc = {p[0],p[1]};
    ...
    if(camera_ == Camera::BOTTOM) ball->fromTopCamera = false;
    else ball->fromTopCamera = true;
    // cout << "Ball detected at: " << ballc->centerX << "," << ballc->centerY <<"  Camera: " << ball->fromTopCamera <<endl;
    // cout << "Ball pan: " << ball->visionBearing << "  Ball distance: " << ball->visionDistance << endl;
}
...
void ImageProcessor::findGoal(int& imageX, int& imageY) {
    ...
    auto blueBlobs = filterBlobs(detected_blobs, c_BLUE, 700);
...
void ImageProcessor::detectRobot() {
    int imageX = -1, imageY = -1;
    findRobot(imageX, imageY);

    WorldObject* robot = &vblocks_.world_object->objects_[WO_TEAM1];
    if(imageX == -1 && imageY == -1){
        robot->seen = false;
        return;
    }
...
void ImageProcessor::findRobot(int& imageX, int& imageY) {
    if(getSegImg() == NULL){
        imageX = -1;
        imageY = -1;
        // cout << "Goal not detected" << endl;
    ...
    auto robotBlobs = filterBlobs(detected_blobs, c_WHITE, 500);
    sort(robotBlobs.begin(), robotBlobs.end(), BlobCompare);
    if(robotBlobs.size() > 0) {
        // cout << "Goal detected at: " << blueBlobs[0].avgX << "\t" << blueBlobs[0].yf << endl;
        double rectArea = (robotBlobs[0].dx) * (robotBlobs[0].dy);
        double density = (robotBlobs[0].lpCount / rectArea);
        if (density > 0.6) {
            imageX = robotBlobs[0].avgX;
            imageY = robotBlobs[0].yf;
        }
        else {
            // cout << "Skipping " << blueBlobs[0].avgX << " " << blueBlobs[0].yf << " " << density << endl;
            imageX = -1;
...
void ImageProcessor::detectLine() {
    if(camera_ == Camera::TOP){return;}
    int imageX = -1, imageYi = -1,imageXi = -1, imageYf = -1,imageXf = -1;
    findLine(imageX, imageXi,imageYi,imageXf,imageYf);

    WorldObject* penaltyline = &vblocks_.world_object->objects_[WO_OWN_PENALTY];
    if(imageX == -1 && imageYf == -1){
        penaltyline->seen = false;
        return;
    }

    penaltyline->imageCenterX = imageX;
    penaltyline->imageCenterY = imageYf;

    Position p = cmatrix_.getWorldPosition(imageX, imageYf);
    ...
    penaltyline->fromTopCamera = (camera_ == Camera::TOP);
    penaltyline->visionPt1 = Point2D(imageXi,imageYi);
    penaltyline->visionPt2 = Point2D(imageXf,imageYf);
    penaltyline->visionLine = LineSegment(penaltyline->visionPt1,penaltyline->visionPt2);
    //cout << "Goal pan: " << goal->visionBearing << "   Goal tilt: " << goal->visionElevation << endl;
    //cout << "Goal distance: " << goal->visionDistance << endl << endl;
    penaltyline->seen = true;
    // cout << "Peanlty line seen with height " << imageYf - imageYi << endl;
   //cout << "Peanlty line seen at distance " << penaltyline->visionDistance<< " and bearing " << penaltyline->visionBearing << endl;
}


void ImageProcessor::findLine(int& imageX,int& imageXi, int& imageYi,int& imageXf, int& imageYf) {
    if(getSegImg() == NULL){
        imageXi = -1;
        imageYi = -1;
        imageXf = -1;
    ...
    auto lineblobs = filterBlobs(detected_blobs, c_WHITE, 700);
    sort(lineblobs.begin(), lineblobs.end(), BlobCompare);
    if(lineblobs.size() > 0) {
        // cout << "Goal detected at: " << blueBlobs[0].avgX << "\t" << blueBlobs[0].yf << endl;
        double rectArea = (lineblobs[0].dx) * (lineblobs[0].dy);
        double density = (lineblobs[0].lpCount / rectArea);
        if ( (lineblobs[0].xf - lineblobs[0].xi > 200.0) && ((density > 0.45 && lineblobs[0].yf - lineblobs[0].yi < 60) || (lineblobs[0].yf - lineblobs[0].yi >= 60))){
            imageX = lineblobs[0].avgX;
            imageYf = lineblobs[0].yf;
            imageXi = lineblobs[0].xi;
            imageXf = lineblobs[0].xf;
            imageYi = lineblobs[0].yi;
        }
        else {
            // cout << "Skipping " << blueBlobs[0].avgX << " " << blueBlobs[0].yf << " " << density << endl;
            // cout << "Line too short or un dense: width " << lineblobs[0].xf - lineblobs[0].xi<< " and height " << lineblobs[0].yf - lineblobs[0].yi << " and density " << density <<  endl;
            imageXi = -1;
            imageYi = -1;
            imageXf = -1;
            imageYf = -1;
